...
func TestEnsureErrorIsGeneratedOnResponse(t *testing.T) {
	...
	_, err = client.sendGetRequest(context.Background(), &GetDetectorOptions{}, nil)
	if err == nil {
...
func TestCreateRequest(t *testing.T) {
	...
	if req.Raw().Header.Get(headerXmsClientRequestId) == "" {
		t.Errorf("Expected uuid in %v header field, but got empty string", headerXmsClientRequestId)
	}

	if req.Raw().Header.Get(headerXmsRequestId) == "" {
...
func TestGetDetector(t *testing.T) {
	...
	_, err := client.sendGetRequest(context.Background(),
		&GetDetectorOptions{
